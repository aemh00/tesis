...
class MLP_last(nn.Module):
    def __init__(self, use_period = False,use_amp = False,in_channels_C=2,in_channels_M=1,c1=128,c2=64, c3=32, c4=16,c5=8, kernel_size=3, hid_dim=32,hid2_dim=16,hid3_dim=8,output_dim=3):
        ...
        self.hidden1 = nn.Linear(c2*1+self.extras, hid_dim, bias=True)
    ...
    def forward(self, x,mask, p,amp, t=0):
        ...
        if self.extras != 0:
            if self.use_period == self.use_amp:
                z = torch.cat((z,p,amp),1)
            elif self.use_period:
                z = torch.cat((z,p),1)
            else:
                z = torch.cat((z,amp),1)
